...
class NetworkMapAgent:
    ...
    def _collect_network_interfaces(self) -> Dict[str, Any]:
        """Collect network interface information with error handling"""
        interfaces_data = {}
        try:
            result = subprocess.run(['ip', 'addr', 'show'], 
                                  capture_output=True, text=True, timeout=30)
            if result.returncode == 0:
                interfaces_data['ip_addr'] = result.stdout
                # Also get interface statistics
                stats_result = subprocess.run(['ip', '-s', 'link'], 
                                            capture_output=True, text=True, timeout=30)
                if stats_result.returncode == 0:
                    interfaces_data['ip_stats'] = stats_result.stdout
        except Exception as e:
            self.logger.warning(f"Failed to get network interfaces: {e}")
        return interfaces_data
    
    def _collect_routing_table(self) -> List[str]:
        ...
        try:
            result = subprocess.run(['ip', 'route', 'show'], 
                                  capture_output=True, text=True, timeout=30)
            if result.returncode == 0:
                routing_data = result.stdout.strip().split('\n')
        except Exception as e:
            self.logger.warning(f"Failed to get routing table: {e}")
        return routing_data
    
    def _collect_arp_table(self) -> List[str]:
        ...
        try:
            result = subprocess.run(['arp', '-a'], 
                                  capture_output=True, text=True, timeout=30)
            if result.returncode == 0:
                arp_data = result.stdout.strip().split('\n')
        except Exception as e:
            self.logger.warning(f"Failed to get ARP table: {e}")
        return arp_data
    
    def _collect_listening_ports(self) -> List[str]:
        """Collect listening ports information"""
        ports_data = []
        try:
            result = subprocess.run(['ss', '-tlnp'], 
                                  capture_output=True, text=True, timeout=30)
            if result.returncode == 0:
                ports_data = result.stdout.strip().split('\n')
        except Exception as e:
            self.logger.warning(f"Failed to get listening ports: {e}")
        return ports_data
    
    def _collect_active_connections(self) -> List[str]:
        """Collect active connections information"""
        connections_data = []
        try:
            result = subprocess.run(['ss', '-tuanp'], 
                                  capture_output=True, text=True, timeout=30)
            if result.returncode == 0:
                connections_data = result.stdout.strip().split('\n')
        except Exception as e:
    ...
    def _collect_network_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if result.returncode == 0:
                stats_data['net_dev'] = result.stdout
        except Exception as e:
            self.logger.warning(f"Failed to get network stats: {e}")
        return stats_data
    
    def _collect_process_network(self) -> List[str]:
