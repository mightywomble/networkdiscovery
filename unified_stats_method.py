    def get_unified_dashboard_stats(self):
        ...
        try:
            ...
            cursor.execute("SELECT version, COUNT(*) FROM agents WHERE version IS NOT NULL GROUP BY version ORDER BY COUNT(*) DESC LIMIT 1")
            result = cursor.fetchone()
            stats['current_deployed'] = result[0] if result else 'Unknown'
            
            # Get latest available version from agent settings or default
            cursor.execute("SELECT value FROM application_settings WHERE key = %s", ('latest_agent_version',))
            result = cursor.fetchone()
